...
//signature: Realtech
const uint8_t RTK_ECO_PATCH_SIGNATURE[8]={0x52,0x65,0x61,0x6C,0x74,0x65,0x63,0x68};
...
struct rtk_eco_patch_entry{
	uint16_t chipID;
	uint16_t patch_length;
	uint32_t start_offset;
} __attribute__ ((packed));

struct rtk_eco_patch{
    uint8_t signature[8];
	uint32_t fm_version;
	uint16_t number_of_total_patch;
	struct rtk_eco_patch_entry entry[0];
} __attribute__ ((packed));

struct rtk_eco_extension_entry{
	uint8_t opcode;
	uint8_t length;
	uint8_t *data;
} __attribute__ ((packed));
...
void rtk_get_eco_timeout(int sig)
{
	ALOGE("RTK get eco timeout\n");
	need_download_fw = 0;
...
/**
* Send vendor cmd to get eco version: 0xfc6d
* If Rom code does not support this cmd, use default a-cut.
*/
void rtk_get_eco_version(void)
{
	...
	ALOGI("bt vendor lib: Get eco version");									
    struct sigaction sa;
    uint8_t *p = (uint8_t *) (p_buf + 1);
    ...
    *p++ = 0; /* parameter length */   
	...
	sa.sa_handler = rtk_get_eco_timeout;
...
//add for multi eco patch
	uint8_t is_multi_eco_patch = 0;
    uint8_t* eco_patch_buf = NULL;
	int eco_patch_length = -1;
	struct rtk_eco_patch* eco_patch_info = NULL;
	struct rtk_eco_patch_entry current_entry;
    ...
    if (bt_vendor_cbacks) //a-cut fc6d status==1
    {
        p_buf = (HC_BT_HDR *) bt_vendor_cbacks->alloc(BT_HC_HDR_SIZE + \
                                                       HCI_CMD_MAX_LEN);
	}
        ...
        switch (hw_cfg_cb.state)
        {
            case HW_CFG_START:
            {
				if((lmp_version != ROM_LMP_8723a)&&(p_mem != NULL))
				{
					...
					if(status == 0)
					{
						gEcoVersion = *((uint8_t *)(p_evt_buf + 1) + HCI_EVT_CMD_CMPL_STATUS_RET_BYTE + 1); 			 
					}
					else if(1 == status)
					{
						gEcoVersion = 0;  //default a-cut
					}
					else
					{
					...
					ALOGI("bt vendor lib: READ_RTK_ROM_VERISION status:%i, gEcoVersion:%i", status, gEcoVersion);
				}
		  ...
		  gNeedToSetHWFlowControl = 0;		
                ...
                buf_len = rtk_get_bt_firmware(&eco_patch_buf, config_len);
                if (buf_len < 0)
                {
                ...
                {			
                	if(lmp_version == ROM_LMP_8723a)
					{
						if(memcmp(eco_patch_buf, RTK_ECO_PATCH_SIGNATURE, 8) == 0)
						{
						...
						else
						{
							...
							else
							{
								...
								memcpy(buf,eco_patch_buf,buf_len);
								free(eco_patch_buf);
								eco_patch_buf = NULL;
								if (config_len)
								{
					...
					{
				    	struct rtk_eco_extension_entry patch_lmp;
  						//check Extension Section Field 
  						if(memcmp(eco_patch_buf + buf_len-config_len-4 ,Extension_Section_SIGNATURE,4) != 0)
  						{
						...
						{
							uint8_t *temp;  
							temp = eco_patch_buf+buf_len-config_len-5;
							do{
								if(*temp == 0x00)
								{
							...
							else
							{
								...
								{
									int i = 0;
									eco_patch_info = (struct rtk_eco_patch*)eco_patch_buf;
									ALOGI("fm_version = 0x%x",eco_patch_info->fm_version); 
									ALOGI("number_of_total_patch = %d",eco_patch_info->number_of_total_patch);
							
									//get right eco patch entry
									for(i; i<eco_patch_info->number_of_total_patch; i++)
									{
										if(*(uint16_t*)(eco_patch_buf+14+2*i) == gEcoVersion + 1)
										{
											current_entry.chipID = gEcoVersion + 1;
											current_entry.patch_length = *(uint16_t*)(eco_patch_buf+14+2*eco_patch_info->number_of_total_patch+2*i);
											current_entry.start_offset = *(uint32_t*)(eco_patch_buf+14+4*eco_patch_info->number_of_total_patch+4*i);
											break;
										}
									}
									ALOGI("chipID = %d(1:a-cut, 2:b-cut, 3:c-cut...)",current_entry.chipID);  
									ALOGI("patch_length = 0x%x",current_entry.patch_length); 
									ALOGI("start_offset = 0x%x",current_entry.start_offset); 
													
									//get right eco version patch: buf, buf_len
									buf_len = current_entry.patch_length + config_len;
									ALOGI("buf_len = 0x%x",buf_len);
													
									if (!(buf = malloc(buf_len))) {
										ALOGE("Can't alloc memory for multi-eco fw&config, errno:%d", errno);
										buf_len = -1;
									}
									else
									{
										memcpy(buf,&eco_patch_buf[current_entry.start_offset],current_entry.patch_length);
										memcpy(&buf[current_entry.patch_length-4],&eco_patch_info->fm_version,4);
									}
									free(eco_patch_buf);
									eco_patch_buf = NULL;
